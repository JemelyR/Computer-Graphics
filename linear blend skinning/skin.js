...
function computeDistanceToLine(pt, vertex0, vertex1) {
	var a = vertex1.subtract(vertex0);
	var b = pt.subtract(vertex0);
	...
	if (proj < 0) {
		var res = pt.subtract(vertex0);
		return res.length();

	  } 
	else if (proj > a.length()) {
		var res = pt.subtract(vertex1);
		return res.length();
	  }

	  var bL = b.length();

	  var final = Math.sqrt(bL*bL - proj * proj);
...
SkinMesh.prototype.rigidSkinning = function () {
	...
	for(var i = 0; i < this.getNumVertices(); i++){
		...
		var bindM = joint.getBindingMatrix();
		var worldM = joint.getWorldMatrix();
		var boneT = worldM.multiply(bindM);

		var transV = boneT.transformPoint(vertex);
		this.setTransformedVertex(i, transV);
	}
...
SkinMesh.prototype.computeLinearBlendedWeights = function () {
	...
	for(var i = 0; i < this.getNumVertices(); i++){
		...
		for(var j =0; j < this.mSkeleton.getNumJoints(); j++){
			...
			var endp = joint.getJointEndPoints();

			var dis = computeDistanceToLine(vertex, endp.v0, endp.v1);

			var weight = 1 / Math.pow(dis, 4);
		...
		for (var j = 0; j < this.mSkeleton.getNumJoints(); j++) {
			...
			this.setVertexWeight(i, j, weight/sum);
		  }
	}
...
SkinMesh.prototype.linearBlendSkinning = function () {
	...
	for(var i = 0; i < this.getNumVertices(); i++){
		...
		for(var j = 0; j < this.mSkeleton.getNumJoints(); j++){
			...
			var transV = boneT.transformPoint(currentPos);
		
			transformedPos = transformedPos.add(transV.multiply(weight));
		}
