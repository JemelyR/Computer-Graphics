...
Joint.prototype.getLocalMatrix = function () {
	...
	var rotM = Matrix.rotate(this.mJointAngle, 0, 0, 1);

	var frameM = Matrix.frame(this.mForward, this.mUp, this.mJointAxis, this.mPosition);
	localM = frameM.multiply(rotM);
...
Joint.prototype.getWorldMatrix = function () {
	var worldT = new Matrix();
	var localT = this.getLocalMatrix();
	if(this.mParent !== null){
        worldT = this.mParent.getWorldMatrix();
		localT = worldT.multiply(localT);
...
// ################ Edit your code below
	var bindM = this.getWorldMatrix();
	var result = bindM.inverse();
	this.mBindingMatrix = result;
// ################
